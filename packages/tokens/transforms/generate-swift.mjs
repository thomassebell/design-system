 ...
 * combination produced by build.mjs – controlled by BRANDS[0] in
 * brands.config.js) and emits a SwiftUI-friendly DesignTokens.swift into the
 * ios-tokens package's Sources/ tree. The generated file is COMMITTED to git –
 * Swift Package Manager consumes packages straight from the repo, so the
 * sources must be checked in. CI regenerates and fails if the committed copy
 * is out of date.
 *
 * Output layout:
...
import { DEFAULT_BRAND, DEFAULT_DENSITY } from "../brands.config.js";

const TOKENS_PATH = "dist/tokens.json";
const OUTPUT_PATH = "../ios-tokens/Sources/DSTokens/DesignTokens.swift";
...
const lines = [
  ...
  `// Source: packages/tokens/dist/tokens.json (${DEFAULT_BRAND} × ${DEFAULT_DENSITY}).`,

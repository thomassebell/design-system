...
| `Text` | `typography.*`, `color.text.*` | **Mirrors the Figma `typography` page one-for-one.** Six `display1`…`display6` (= `header/display 1`…`6`, header family per the size-to-family rule above) plus `body` (= `paragraph/paragraph`) and `bodySmall` (= `paragraph/small`). No other variants – `caption` and `overline` were removed 2026-08-18 as code-only inventions. **A variant is a style, never an element** – every variant renders `<p>`, and heading level is set explicitly with `as`, because document hierarchy and visual size are independent decisions. **Emphasis is not a variant.** Figma's `paragraph emphasized` / `small emphasized` exist because Figma cannot set a weight on a text range; in code the same intent is a nested `<strong>`/`<b>` (bound to `font-weight.emphasized`, not the browser's bold) for a word, or the `weight` prop for a whole block |
